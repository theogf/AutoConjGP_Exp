function initial_lengthscale(X)
    if size(X, 1) > 10000
        D = pairwise(
            SqEuclidean(),
            X[sample(1:size(X, 1), 10000, replace = false), :],
            dims = 1,
        )
    else
...
function run_nat_grads_with_adam(
    model,
    iterations,
    X_test,
    y_test,
    LogArrays;
    ind_points_fixed = true,
    kernel_fixed = false,
    callback = nothing,
    Stochastic = true,
    time_max = Inf,
    ...
    gamma_incremented = tf.where(
        tf.less(gamma, gamma_max),
        tf.minimum(gamma * gamma_step, gamma_max),
        gamma_max,
    )
    ...
    op_natgrad = gpflow.training.NatGradOptimizer.(gamma = gamma).make_optimize_tensor(
        model,
        var_list = var_list,
    )
    op_adam = 0
    ...
    return model.anchor(sess)
end

function run_grads_with_adam(
    model,
    iterations,
    X_test,
    y_test,
...
function testloglikelihood(model::AbstractGP, y_test, y_predic)
    if isa(model.likelihood, ClassificationLikelihood)
        return mean(vcat(
            log.(max.(y_predic[y_test.==1], 1e-8)),
            log.(max.(1.0 .- y_predic[y_test.==-1], 1e-8)),
        ))
    elseif isa(model.likelihood, RegressionLikelihood)
...
function cbcavi(X_test, y_test, LogArrays)
    return function callback(model, iter)
...
function AugmentedGaussianProcesses.ELBO(model, sess = model.enquire_session())
    return sess.run(model.likelihood_tensor)
end
...
function csv2h5(file_name::String, classification = false)
    ...
    return h5write(file_short * ".h5", "data", data)
end

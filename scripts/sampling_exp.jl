...
Distributions.logpdf(d::Matern32Turing, x::Real) =
    log(one(x) + sqrt(3 * abs2(x - d.μ))) - sqrt(3 * abs2(x - d.μ))
...
likelihood2gibbs = Dict(
    :Logistic => LogisticLikelihood(),
    :Laplace => LaplaceLikelihood(β_l),
    :StudentT => StudentTLikelihood(ν),
    :Matern32 => Matern3_2Likelihood(),
)
likelihood2turing = Dict(
    :Logistic => logisticmodel,
    :Laplace => laplacemodel,
    :StudentT => studenttmodel,
    :Matern32 => matern32model,
)
likelihood2ptype = Dict(
...
function sample_exp(dict = defaultdictsamp)
    ...
    base_file =
        datadir("datasets", string(likelihood2ptype[likelihood]), "small", file_name)
    ...
    if doGibbs
        ...
        for i in 1:nChains
            ...
            amodel = VGP(
                ...
                y,
                kernel,
                likelihood2gibbs[likelihood],
                GibbsSampling(samplefrequency = 1, nBurnin = 0),
                verbose = 2,
                optimizer = false,
            )
        ...
        infos = DataFrame(
            reshape(["GS", nSamples, mean(times_Gibbs), 0, 0, 0, 0], 1, :),
            [:alg, :nSamples, :time, :epsilon, :n_step, :n_adapt, :accept],
        )
        @tagsave(
            datadir("part_1", string(likelihood), savename("GS", params_samp, "bson")),
            @dict chains infos
        )
    ...
    if doHMC
        ...
        for i in 1:nChains
            ...
            t = @elapsed HMCchain = sample(
                likelihood2turing[likelihood](X, y_turing, L),
                HMC(epsilon, n_step),
                nSamples,
            )
        ...
        infos = DataFrame(
            reshape(["HMC", nSamples, mean(times_HMC), epsilon, n_step, 0, 0], 1, :),
            [:alg, :nSamples, :time, :epsilon, :n_step, :n_adapt, :accept],
        )
        params_HMC = @ntuple(nSamples, epsilon, n_step)
        @tagsave(
            datadir("part_1", string(likelihood), savename("HMC", params_HMC, "bson")),
            @dict chains infos
        )
        push!(all_chains, chains)
    ...
    if doNUTS
        ...
        for i in 1:nChains
            @info "Turing chain $i/$nChains"
            t = @elapsed NUTSchain = sample(
                likelihood2turing[likelihood](X, y_turing, L),
                NUTS(nSamples, n_adapt, accept),
            )
        ...
        infos = DataFrame(
            reshape(["NUTS", nSamples, mean(times_NUTS), 0, 0, n_adapt, accept], 1, :),
            [:alg, :nSamples, :time, :epsilon, :n_step, :n_adapt, :accept],
        )
        params_NUTS = @ntuple(nSamples, n_adapt, accept)
        @tagsave(
            datadir("part_1", string(likelihood), savename("NUTS", params_NUTS, "bson")),
            @dict chains infos
        )
        push!(all_chains, chains)
    ...
    if doMH
        ...
        for i in 1:nChains
            @info "Turing chain $i/$nChains"
            t = @elapsed MHchain =
                sample(likelihood2turing[likelihood](X, y_turing, L), MH(nSamples))
            push!(MHchains, MHchain)
            push!(times_MH, t)
        end
        chains = DataFrame(chainscat(MHchains...))
        infos = DataFrame(
            reshape(["MH", nSamples, mean(times_MH), 0, 0, 0, 0], 1, :),
            [:alg, :nSamples, :time, :epsilon, :n_step, :n_adapt, :accept],
        )
        @tagsave(
            datadir("part_1", string(likelihood), savename("MH", params_samp, "bson")),
            @dict chains infos
        )
        push!(all_chains, chains)
        push!(times, times_MH)

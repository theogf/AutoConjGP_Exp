...
try
    ...
    likelihood_GD = Dict(
        :Logistic => py"BernoulliLogit()",
        :Matern32 => py"Matern32()",
        :Laplace => py"Laplace()",
        :StudentT => py"gpflow.likelihoods.StudentT(3.0)",
    )
    global likelihood2problem = Dict(
        "BernoulliLogit" => :classification,
        "Matern32" => :regression,
        "Matern32" => :regression,
        "StudentT" => :regression,
        "Laplace" => :regression,
    )
    likelihood_CAVI = Dict(
        :Logistic => LogisticLikelihood(),
        :Matern32 => GenMatern32Likelihood(),
        :Laplace => LaplaceLikelihood(),
        :StudentT => StudentTLikelihood(3.0),
    )
    ...
    params = @ntuple(likelihood, nMinibatch, nInducing, nIter)
    for ((X_train, y_train), (X_test, y_test)) in kfolds((X, y), 10, obsdim = 1)
        Z = AugmentedGaussianProcesses.KMeansInducingPoints(
            X_train,
            nInducing,
            nMarkov = 10,
        )
        ...
        if doCAVI
            ...
            model = SVGP(
                ...
                y_train,
                CAVI_kernel,
                likelihood_CAVI[likelihood],
                AnalyticSVI(nMinibatch),
                nInducing,
            )
            train!(model, iterations = 5)
            @info "Finished pre-training"
            ...
            try
                train!(
                    model,
                    iterations = nIter,
                    callback = cbcavi(X_test, y_test, LogArrays),
                )
            catch e
                if !isa(e, InterruptException)
                    rethrow(e)
            ...
            training_df =
                DataFrame(hcat(LogArrays...)', [:i, :t_init, :metric, :nll, :ELBO, :t_end])
            results = DataFrame(
                [
                    [:CAVI],
                    [likelihood],
                ...
                [:model, :likelihood, :nInducing, :nMinibatch, :t_first, :training_df],
            )

            @tagsave(
                datadir("part_3", file_name, savename("CAVI", params, "bson")),
                @dict results
            )
        end
        ## Run NGD
        if doNGD
            ...
            global model = gpflow.models.SVGP(
                X_train,
                Float64.(reshape(y_train, (length(y_train), 1))),
                kern = deepcopy(GD_kernel),
                likelihood = likelihood_GD[likelihood],
                num_latent = 1,
                Z = Z,
                minibatch_size = nMinibatch,
            )
            t_first = time_ns()
            try
            ...
            training_df =
                DataFrame(hcat(LogArrays...)', [:i, :t_init, :metric, :nll, :ELBO, :t_end])
            results = DataFrame(
                [[:NGD], [likelihood], [nInducing], [nMinibatch], [t_first], [training_df]],
                [:model, :likelihood, :nInducing, :nMinibatch, :t_first, :training_df],
            )
            @tagsave(
                datadir("part_3", file_name, savename("NGD", params, "bson")),
                @dict results
            )
        end
        ## Run GD
        if doGD
            ...
            training_df =
                DataFrame(hcat(LogArrays...)', [:i, :t_init, :metric, :nll, :ELBO, :t_end])
            results = DataFrame(
                [[:GD], [likelihood], [nInducing], [nMinibatch], [t_first], [training_df]],
                [:model, :likelihood, :nInducing, :nMinibatch, :t_first, :training_df],
            )
            @tagsave(
                datadir("part_3", file_name, savename("GD", params, "bson")),
                @dict results
            )
        end
        break
